...
with DAG('store_DAG',default_args=default_args,schedule_interval='@daily', template_searchpath=['/usr/local/airflow/sql_files'], catchup=True) as dag:
    ...
    t1=BashOperator(task_id='check_file_exists', bash_command='shasum ~/store_files_airflow/raw_store_transactions_2020-06-30.csv', retries=2, retry_delay=timedelta(seconds=15))
    ...
    t9 = BashOperator(task_id='rename_raw', bash_command='mv ~/store_files_airflow/raw_store_transactions_2020-06-30.csv ~/store_files_airflow/raw_store_transactions_2020-06-31.csv')
